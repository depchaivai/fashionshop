<x-dashboard.adminlayout>
    <x-slot name="myjs">
        <script>
            ...
            const editFSBtn = document.querySelector('#editFSBtn');
            ...
            editFSBtn.addEventListener('click', ()=>{
                ...
                let fsid = editFSBtn.getAttribute('fs-id');
                $.ajax({
                    url: '/dashboard/callajax/fs/update/'+fsid, // <-- point to server-side PHP script 
                    cache: false,
                    contentType: false,
                    processData: false,
                    data: formData,                        
    ...
    <div class="w-full p-4">
        <span class="px-2 py-1 font-bold text-2xl block"><i class="fa-solid fa-plus mr-2"></i>sửa flash sale</span>
        <div class="mt-5">
            <div class="w-full p-10 max-w-[700px]">
                    ...
                    <div id="preBanner" class="mb-6 flex">
                        @if (!empty($fs->banner))
                            <div class="w-[120px]"></div>
                            <img src="{{URL($fs->banner)}}" alt="{{$fs->title}}" class="w-[calc(100%_-_120px)] h-[100px] object-cover">
                        @endif
                        ...
                        <div class="flex w-[calc(100%_-_120px)] items-center">
                            <div class="relative w-[calc(50%-24px)]">
                                ...
                                <input name="start" type="text"
                                    value = "{{Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $fs->from)->format('M d Y')}}"
                            ...
                            <div class="relative w-[calc(50%-24px)]">
                                ...
                                <input name="end" type="text"
                                    value = "{{Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $fs->expire_day)->format('M d Y')}}"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 datepicker-input"
                    ...
                    <div class="mb-6 flex items-center relative">
                        ...
                        <input type="text" id="product" name="product"
                            ...
                            >
                        <ul id="ulProduct"
                            class="absolute top-[50px] bg-slate-100 right-0 w-[calc(100%_-_120px)]">
                            ...
                            <tr class="font-bold capitalize text-blue-500">
                                ...
                                <tbody id="bodydis">
                                    ...
                                    <tr class="productRow">
                                        <td>
                                            <div class="flex items-center">
                                                <img src="{{URL('public/images/'.$sale->product->image)}}" class="w-[50px] h-[60px] object-cover">
                                                <span class="ml-2">{{$sale->product->name}}</span>
                                            </div>
                                        </td>
                                        <td>
                                            <input type="number" max="100" min="0" class="p-1 border-0 rounded-md shadow-inner" value="{{$sale->discount}}" p-id="{{$sale->product->id}}" name="discount">
                                        </td>
                                        <td>
                                            %
                                        </td>
                    ...
                    <div id="editFSBtn" fs-id="{{$fs->id}}" class="w-full flex justify-center"><button
                            class="my-4 px-3 py-1 text-white bg-green-400">thêm</button></div>
